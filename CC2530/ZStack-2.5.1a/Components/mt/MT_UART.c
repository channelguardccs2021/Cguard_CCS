...
void rxCB( uint8 port, uint8 event )
{
    ...
    if(buf[0] == 0x82 || buf[0] == 0x31)
    {
      //ZDOInitDevice( 0 );   //如果串口收到"1",就加入网络，停止闪烁
      ZDOInitDevice( 0 ); 
      //HalUARTWrite(0, buf, len);
      //HalUARTWrite(0, "1", 1);
    }
    if(buf[0] == 0x80 || buf[0] == 0x30)      //如果串口收到"0",就离开网络，清除网络状态的重置
    {
      //HalUARTWrite(0, "ready", 5);
      //SampleApp_Send_CONTROL_Message(); //通知协调器
      //NLME_LeaveReq_t leaveReq;
      //osal_memset((uint8 *)&leaveReq,0,sizeof(NLME_LeaveReq_t));
      //osal_memcpy(leaveReq.extAddr,NLME_GetExtAddr(),Z_EXTADDR_LEN);
      //NLME_LeaveReq( &leaveReq );
      
      afIncomingMSGPacket_t *MSGpkta;
      MSGpkta-> clusterId = Mgmt_Leave_req;
      ((zdoIncomingMsg_t *)MSGpkta)->asdu= NLME_GetExtAddr();
      ZDO_ProcessMgmtLeaveReq((zdoIncomingMsg_t *)MSGpkta);
    ...
    if(len)
    {
        ...
        SampleApp_Send_Control_Message(buf);
        len = 0;
    }     
